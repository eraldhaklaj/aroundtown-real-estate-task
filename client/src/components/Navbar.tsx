import { HouseIcon, LogOutIcon, WandSparklesIcon } from "lucide-react";
      ...
      <div className="mx-auto flex h-14 max-w-7xl items-center gap-2 px-4 sm:gap-4">
        ...
        <DropdownMenu>
          <DropdownMenuTrigger asChild>
            <Button variant="ghost" className="ml-auto h-9 gap-2 px-2" aria-label="Account menu">
              <Avatar className="size-7">
                <AvatarFallback className="text-xs">{initials}</AvatarFallback>
              </Avatar>
              <span className="hidden text-sm md:inline">{user.name}</span>
              <Badge variant={user.role === "agent" ? "default" : "secondary"} className="capitalize">
